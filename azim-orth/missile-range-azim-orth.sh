...
PROJ=G${LON}/${LAT}/6i
REGION=g # g for Globe

gmt pscoast -R${REGION} -J${PROJ} -Bag -Dc -A5000 -Gwhite -SDarkTurquoise -P $INIT > GMT_range.ps
...
gmt pscoast -R${REGION} -J${PROJ} -Bag -E=EU,=AS,=OC,=NA -Dc -A5000 -Gwhite -W0.25p,100/100/100 -P $APPEND >> GMT_range.ps

# Range circles
gmt psxy $RANGE_MIN_GMT -R${REGION} -J${PROJ} -Baf -W0.75p,220/30/40 -Gred -t80 -P $APPEND >> GMT_range.ps
# Leave inner circle intact (not filled with any color) by clipping it
gmt psclip $RANGE_MIN_GMT -R${REGION} -J${PROJ} -N -P -K -O >> GMT_range.ps
gmt psxy $RANGE_MAX_GMT -R${REGION} -J${PROJ} -Baf -W0.75p,220/30/40 -Gred -t70 -P $APPEND >> GMT_range.ps
# Cancel clipping
gmt psclip -C -R -J -Baf $APPEND >> GMT_range.ps

gmt pstext locations.txt -R${REGION} -J${PROJ} -F+f+a+j -P $APPEND >> GMT_range.ps
gmt psxy   locations.txt -R${REGION} -J${PROJ} -Sc0.12c -G204/0/0 -W0.5p -P $APPEND >> GMT_range.ps
gmt pstext select_capitals.txt -R${REGION} -J${PROJ} -F+f+a+j -P $APPEND >> GMT_range.ps
gmt psxy   select_capitals.txt -R${REGION} -J${PROJ} -Ss0.18c -G204/0/0 -W0.5p $APPEND >> GMT_range.ps

# transparency only works in the PDF format
ps2pdf GMT_range.ps GMT_range.pdf
